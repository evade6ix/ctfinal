...
async function waitForScryfallSlot() {
  ...
  const waitMs = Math.max(0, SCRYFALL_DELAY_MS - elapsed);
...
async function findScryfallCard(item, cache) {
  ...
  // CardTrader sometimes has set codes like "cafr" where Scryfall expects "afr".
...
function dedupeManaPoolPayload(rawPayload) {
  const map = new Map();
  ...
  for (const item of rawPayload) {
    ...
    if (!existing) {
      map.set(key, {
        ...item,
        _source_custom_external_ids: [item.custom_external_id],
        _price_conflicts: [],
      });
      continue;
    }
    ...
    existing._source_custom_external_ids.push(item.custom_external_id);

    // Mana Pool can only have one price for this exact product variant.
    // Keep the first item price, because inventoryItems are sorted updatedAt desc.
    if (existing.price_cents !== item.price_cents) {
      existing._price_conflicts.push({
        custom_external_id: item.custom_external_id,
        kept_price_cents: existing.price_cents,
        skipped_price_cents: item.price_cents,
      });
    }
  }

  for (const merged of map.values()) {
    if (merged._source_custom_external_ids.length > 1) {
      duplicateGroups.push({
        scryfall_id: merged.scryfall_id,
        language_id: merged.language_id,
        finish_id: merged.finish_id,
        condition_id: merged.condition_id,
        final_quantity: merged.quantity,
        kept_custom_external_id: merged.custom_external_id,
        merged_custom_external_ids: merged._source_custom_external_ids,
        price_conflicts: merged._price_conflicts,
      });
    }

    delete merged._source_custom_external_ids;
    delete merged._price_conflicts;
  }
...
async function main() {
  ...
  const rawPayloadCount = payload.length;
  const deduped = dedupeManaPoolPayload(payload);
  payload = deduped.payload;

  console.log("\n========================================");
  console.log("DUPLICATE CONSOLIDATION");
  console.log("========================================");
  console.log(`Raw payload items before merge: ${rawPayloadCount}`);
  console.log(`Payload items after merge: ${payload.length}`);
  console.log(`Duplicate product groups merged: ${deduped.duplicateGroups.length}`);

  if (deduped.duplicateGroups.length) {
    console.log("\nDuplicate groups preview:");
    console.log(JSON.stringify(deduped.duplicateGroups.slice(0, 20), null, 2));
  }
